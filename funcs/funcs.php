<?php
	...
	function isNull($nombre, $pass, $pass_con, $email, $telefono){
		if(strlen(trim($nombre)) < 1 ||  strlen(trim($pass)) < 1 || strlen(trim($pass_con)) < 1 || strlen(trim($email)) < 1 || strlen(trim($telefono)) < 1)
		{
			return true;
			} else {
			return false;
...
// Incluye tus credenciales de conexión a la base de datos aquí
$host = "localhost";
$usuario = "todosopo";
$contrasena = ":zhO2]TFF4g3l0";
$base_de_datos = "todosopo_datavoic_support";

// Función para obtener el ID de usuario por nombre
function obtenerIdUsuarioPorNombre($nombreUsuario) {
    global $host, $usuario, $contrasena, $base_de_datos;
    ...
    // Evita inyección SQL escapando el nombre de usuario
    $nombreUsuario = $mysqli->real_escape_string($nombreUsuario);
    ...
    if ($result->num_rows === 1) {
        ...
        // El usuario no existe, maneja el error como mejor te parezca
        $idUsuario = 0; // Puedes establecer un valor predeterminado o un código de error
    }
...
function enviarEmail($email, $nombre, $asunto, $cuerpo){
	...
	function login($usuario, $password)
	{
		...
		$errors = ""; // Inicializa la variable $errors
